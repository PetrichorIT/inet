...
fn v4_empty() -> std::io::Result<()> {
    ...
    let output = Ipv4Packet::from_buffer(input.to_buffer()?)?;
...
fn v6_empty() -> std::io::Result<()> {
    ...
    let output = Ipv6Packet::from_buffer(dbg!(input.to_buffer()?))?;
    assert_eq!(input, output);
    ...
    let output = Ipv6Packet::from_buffer(input.to_buffer()?)?;
    assert_eq!(input, output);
...
fn ipv4_addr() {
    ...
    assert_eq!(
        addr,
        Ipv4Addr::from_buffer(addr.to_buffer().unwrap()).unwrap()
    );
