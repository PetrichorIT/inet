...
impl<W: Write, I: PartialEq + Clone> DefaultBlockWriter<W, I> {
    ...
    pub fn new(mut output: W, appl_name: &str) -> Result<Self> {
        ...
        output.write_all(&shb.write_to_bytes_mut()?)?;
        Ok(Self {
            output,
            interfaces: Vec::new(),
            packet_count: 0,
    ...
    fn write_zero_packet(&mut self) -> Result<()> {
        ...
        self.output.write_all(&ebp.write_to_bytes_mut()?)
    }
}
...
impl<W: Write, I: PartialEq + Clone> BlockWriter<I> for DefaultBlockWriter<W, I> {
    fn add_interface(
        ...
        self.output.write_all(&idb.write_to_bytes_mut()?)?;
    ...
    fn add_packet(
        ...
        self.packet_count += 1;
        self.output.write_all(&epb.write_to_bytes_mut()?)?;
