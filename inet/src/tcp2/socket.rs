...
use crate::tcp2::stream::Inner;
use crate::tcp2::{Config, State};
use crate::IOContext;
...
impl TcpSocket {
    ...
    pub fn new_v4() -> Result<TcpSocket> {
        IOContext::with_current(|ctx| {
            Ok(TcpSocket {
                ...
                config: RefCell::new(ctx.tcp2.config.clone()),
    ...
    pub async fn connect(mut self, peer: SocketAddr) -> Result<TcpStream> {
        ...
        let fd = IOContext::with_current(|ctx| {
            ctx.tcp2_connect(peer, Some(self.config.borrow().clone()), Some(fd))
        })?;
