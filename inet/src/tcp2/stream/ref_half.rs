...
impl ReadHalf<'_> {
    ...
    pub async fn peek(&self, buf: &mut [u8]) -> Result<usize, Error> {
        loop {
            ...
            match IOContext::with_current(|ctx| ctx.tcp2_peek(self.stream.inner.fd, buf)) {
                Ok(n) => return Ok(n),
    ...
    pub fn try_read(&self, buf: &mut [u8]) -> Result<usize, Error> {
        IOContext::with_current(|ctx| ctx.tcp2_read(self.stream.inner.fd, buf))
    }
...
impl WriteHalf<'_> {
    ...
    pub fn try_write(&self, buf: &[u8]) -> Result<usize, Error> {
        IOContext::with_current(|ctx| ctx.tcp2_write(self.stream.inner.fd, buf))
    }
...
impl AsyncRead for ReadHalf<'_> {
    fn poll_read(
        ...
        IOContext::with_current(|ctx| {
            ctx.tcp2_poll_read(self.stream.inner.fd, cx, buf)
...
impl AsyncWrite for WriteHalf<'_> {
    fn poll_write(
        ...
        IOContext::with_current(|ctx| ctx.tcp2_poll_write(self.stream.inner.fd, cx, buf))
    }
    fn poll_flush(
        self: std::pin::Pin<&mut Self>,
        cx: &mut std::task::Context<'_>,
    ) -> std::task::Poll<Result<(), std::io::Error>> {
        IOContext::with_current(|ctx| ctx.tcp2_flush(self.stream.inner.fd, cx))
    }
    fn poll_shutdown(
        self: std::pin::Pin<&mut Self>,
