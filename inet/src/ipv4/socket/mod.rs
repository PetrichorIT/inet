...
use types::{
    icmpv4::{IcmpV4Packet, IcmpV4Type},
...
struct Envelope {
    ifid: IfId,
    pkt_or_error: io::Result<Ipv4Packet>,
}
...
impl RawV4Socket {
    ...
    pub async fn recv(&mut self) -> io::Result<Ipv4Packet> {
        self.rx
            .recv()
            .await
    ...
    pub fn try_recv(&mut self) -> io::Result<Ipv4Packet> {
        self.rx
            .try_recv()
...
impl RawV4SocketHandle {
    pub(super) fn recv(&mut self, ifid: IfId, pkt: Ipv4Packet) {
        if let Err(err) = self.tx.try_send(Envelope {
            ifid,
            pkt_or_error: Ok(pkt),
        }) {
            tracing::error!("raw socket dropped: {err:?}");
        }
    }
...
impl IOContext {
    pub(crate) fn ipv4_raw_socket_link_update(&mut self, fd: Fd) {
        let Some(sockets) = self.ipv4.sockets.get_mut(&fd) else {
            return;
        };
    ...
    pub(super) fn ipv4_raw_socket_on_icmp(
        &mut self,
        ifid: IfId,
        fd: Fd,
        msg: &IcmpV4Packet,
        ...
        let Some(handle) = self.ipv4.sockets.get_mut(&fd) else {
            return;
        };
        ...
        let should_report =
            handle.all_icmp || matches!(msg.typ, IcmpV4Type::DestinationUnreachable { .. });

        if should_report {
            let error = Error::new(ErrorKind::ConnectionRefused, msg.typ.as_error_string());
            handle.recv_error(ifid, error);
        }
    }

    fn ipv4_raw_socket_create(&mut self, proto: u8) -> io::Result<(Fd, Receiver<Envelope>)> {
        let fd = self.socket_create(SocketDomain::AF_INET, SocketType::SOCK_RAW, proto as i32)?;

        let (tx, rx) = channel(16);

        self.ipv4.sockets.insert(
            fd,
            RawV4SocketHandle {
                all_icmp: false,
                proto,
