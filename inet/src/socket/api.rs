...
pub fn socket(domain: SocketDomain, typ: SocketType, protocol: i32) -> Result<Fd> {
    IOContext::failable_api(|ctx| ctx.socket(domain, typ, protocol))
}
...
pub fn bind(sockfd: Fd, addr: SocketAddr) -> Result<()> {
    IOContext::failable_api(|ctx| ctx.socket_bind(sockfd, addr))?;
    Ok(())
}
...
pub fn close(fd: Fd) -> Result<()> {
    IOContext::failable_api(|ctx| ctx.socket_close(fd))
}
