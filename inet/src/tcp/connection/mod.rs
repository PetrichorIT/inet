...
impl Connection {
    ...
    pub fn on_icmp_v4(&mut self, icmp: &IcmpV4Packet, contained: &Ipv4Packet) -> io::Result<()> {
        if contained.proto != PROTO_TCP {
    ...
    fn icmpv4_into_soft_error(pkt: &IcmpV4Packet) -> Error {
        // TODO: better impl
        Error::other(format!("{pkt:?}"))
    }

    pub fn on_icmp_v6(&mut self, icmp: &IcmpV6Packet, contained: &Ipv6Packet) -> io::Result<()> {
        // Only consider TCP2 packets
        if contained.proto != PROTO_TCP {
            return Ok(());
        }
        ...
        let inferred_quad = Quad {
            src: SocketAddr::new(contained.src.into(), header_part.src),
            dst: SocketAddr::new(contained.dst.into(), header_part.dst),
        };
        if inferred_quad != self.quad {
            return Ok(());
        }
    ...
    fn icmpv6_classify(pkt: &IcmpV6Packet) -> IcmpAction {
        use IcmpV6DestinationUnreachableCode::*;
        use IcmpV6Packet::*;

        // TODO: DestinationUnreachable(AddressUnreachable) is classfified as both a hard and a soft error
        match pkt {
            DestinationUnreachable(inner)
                if matches!(inner.code, NoRouteToDestination | AddressUnreachable) =>
            {
                IcmpAction::SoftError
            }
            DestinationUnreachable(inner)
                if matches!(
                    inner.code,
                    PortUnreachable | AddressUnreachable | BeyondScopeOfSourceAddress
                ) =>
            {
                IcmpAction::HardError
            }
            TimeExceeded(_) => IcmpAction::SoftError,
            ParameterProblem(_) => IcmpAction::SoftError,
            _ => IcmpAction::Ingore,
        }
    }
